...
function dateToTimestamp(date) {
  const old = new Date(null);
  const chooseDate = new Date(date);
  return chooseDate - old;
}
...
function getTime(date) {
  return new Date(date).toTimeString().split(' ').at(0);
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= days; i += 1) {
    const dataCheck = new Date(year, month - 1, i).getDay();
    if (dataCheck === 0 || dataCheck === 6) res += 1;
  }
  return res;
}
...
function isLeapYear(date) {
  const year = date.getFullYear();
  let res = false;
  if (year % 4 === 0 && year % 100 !== 0) res = true;
  else if (year % 400 === 0) res = true;
  return res;
}
